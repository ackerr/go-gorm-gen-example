...
func Test_userQuery(t *testing.T) {
	...
	err = do.Create(&model.User{})
	if err != nil {
		t.Error("create item in table <user> fail:", err)
	}

	err = do.Save(&model.User{})
	if err != nil {
		t.Error("create item in table <user> fail:", err)
	}

	err = do.CreateInBatches([]*model.User{{}, {}}, 10)
	if err != nil {
		t.Error("create item in table <user> fail:", err)
	}

	_, err = do.Select(user.ALL).Take()
	if err != nil {
	...
	err = do.Where(primaryKey.IsNotNull()).FindInBatches(&[]*model.User{}, 10, func(tx gen.Dao, batch int) error { return nil })
	if err != nil {
		t.Error("FindInBatches() on table <user> fail:", err)
	}

	_, err = do.Select(user.ALL).Where(primaryKey.IsNotNull()).Order(primaryKey.Desc()).Find()
	if err != nil {
	...
	_, err = do.Select(user.ALL).Omit(primaryKey).Take()
	if err != nil {
	...
	_, err = do.ScanByPage(&model.User{}, 0, 1)
	if err != nil {
